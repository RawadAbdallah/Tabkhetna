...
const getEngineeredPrompt = (title, listOfCurrentInfo, missing) => {
    ...
    return `As a food chef, generate a basic set of instructions for a ${title} with the provided ${currentInfo}. Please provide clear and concise steps, avoiding additional phrases. Ensure that no comma is used within a sentence; use a comma only when separating each ${missing}. Omit numbers or dashes as items are separated by commas. Format the instructions as follows: (${missing}, ${missing}, ...). Keep it within 300 characters. I repeat commas are only written when item seperation is needed, for example: "Grab the milk, water and the eggs " should be without commas like this "Grab the milk water and the eggs".${currentInfo}: ${listOfCurrentInfo}`;
};  

const generateRecipeMissingInfo = async (req, res) => {
    let { title, listOfCurrentInfo, missing } = req.body;
    const prompt = getEngineeredPrompt(
        title,
        listOfCurrentInfo,
        missing
    );
